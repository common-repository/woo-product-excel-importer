...
function woopei_importProducts() {
	...
	if ( $_SERVER['REQUEST_METHOD'] == 'POST' && current_user_can( 'wpeieWoo' ) && isset( $_POST['importProducts'] ) ) {
		...
		if ( $_FILES['file']['size'] > 0 ) {
			...
			if ( $_FILES['file']['type'] == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' ) {
				...
				<form method='POST' id ='product_process' action= "<?php print admin_url( 'admin.php?page=woo-product-importer' ); ?>">
...
function woopei_process() {
	...
	if ( isset( $_POST['finalupload'] ) && current_user_can( 'wpeieWoo' ) ) {
		...
		if ( post_exists( $title ) === 0 ) {
					...
					print "<p class='success'><a href='" . esc_url( get_permalink( $id ) ) . "' target='_blank'>" . $title . '</a> ' . esc_html__( 'created', 'woo-product-excel-importer' ) . '.</p>';
		} else {
					// update
					...
					'post_type'    => 'product',
						...
						$post = array(
							...
							'post_type'    => 'product',
									...
									wp_update_post( $post );
									print "<p class='warning'><a href='" . esc_url( get_permalink( $id ) ) . "' target='_blank'>" . $title . '</a> ' . esc_html__( 'already exists. Updated', 'woo-product-excel-importer' ) . '.</p>';
		}
		...
		if ( $i === $finalUpload ) {
			...
			print "<div class='importMessage'>
					<h2>" . esc_html( $i ) . ' / ' . esc_html( $finalUpload ) . ' ' . esc_html__( 'Please dont close this page... Loading...', 'wpeiePro' ) . "</h2>
						<p>
